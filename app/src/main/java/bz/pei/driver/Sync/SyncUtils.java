...
import bz.pei.driver.Sync.common.accounts.GenericAccountService;
import bz.pei.driver.Sync.provider.FeedContract;
...
public class SyncUtils {
    ...
    public static final String ACCOUNT_TYPE = "bz.pei.driver.Sync.account";
...
//        b.putBoolean(ContentResolver.SYNC_EXTRAS_EXPEDITED, true);
    ...
    public static boolean checkIfSyncEnabled(Context context) {
        if (ContentResolver.getIsSyncable(GenericAccountService.GetAccount(ACCOUNT_TYPE), FeedContract.CONTENT_AUTHORITY) <= 0) {
            ...
            // sync is not enable
            return false;
        } else if (!ContentResolver.getSyncAutomatically(GenericAccountService.GetAccount(ACCOUNT_TYPE), FeedContract.CONTENT_AUTHORITY)) {
            return false;
        }
        return true;
    }
