...
public class ShareTripViewModel extends BaseNetwork<RequestModel, ShareTripNavigator> implements GoogleApiClient.ConnectionCallbacks,
    ...
    private long waitingtimeBeforeTrip = 0L, waitingtimeInTrip = 0L, totalWaitingTime = 0L;
    HashMap<String, String> map = new HashMap<>();

    public ShareTripViewModel(GitHubService gitHubService, GitHubMapService gitHubMapService, SharedPrefence sharedPrefence, Gson gson, Socket socket, LocationRequest locationRequest, DrawerAct drawerAct) {
...
//        kilometerString = drawerAct.getString(R.string.text_km);
        isPathDrawn = false;
        getDriverDetails();

    }

    public void setUpTripDetails(RequestModel.Request model) {
        if (model == null)
            return;

        if (model != null) {
            pickupLatlng.set(model.getPickupLatlng());
            dropLatlng.set(model.getDropLatlng());
  /*          tripPaymentMode.set(
                    model.request.payment_opt == 1 ?
                            drawerAct.getString(R.string.txt_cash) :
                            model.request.payment_opt == 0 ?
                                    drawerAct.getString(R.string.txt_card) :
                                    drawerAct.getString(R.string.text_wallet));*/
            tripPaymentMode.set(drawerAct.getString(
                    model.payment_opt == 1 ? R.string.txt_cash :
                            model.payment_opt == 0 ? R.string.txt_card :
                                    model.payment_opt == 4 ? R.string.text_corporate :
                                            R.string.text_wallet));
            isArrived.set(model.is_driver_arrived == 1);
            isTripStarted.set(model.is_trip_start == 1);
            isCompleted.set(model.is_completed == 1);
            request_ID = model.id + "";
            sharedPrefence.saveIntValue(SharedPrefence.REQUEST_ID, model.id);
            if (model.user != null) {
                user_ID = model.user.id + "";
                userName.set(model.user.firstname + " " + model.user.lastname);
                user_PhoneNumber = model.user.phoneNumber;
                if (model.user.review > 0)
                    userRating.set((int) model.user.review);
                if (!CommonUtils.IsEmpty(model.user.profilePic))
                    userPic.set(model.user.profilePic);
...
//    TripLocationSocketModel socketModel = new TripLocationSocketModel();
...
//        isCompleted.set(true);
    ...
    public void setWaitingTime(int time, Integer seconds) {
        startLocationUpdate();
        if (isTripStarted.get())
            waitingtimeInTrip = waitingtimeBeforeTrip > 0 ? (waitingtimeBeforeTrip - time) : time;
        else
